...
if (inode_offset_byte == bucket_h_inode->next) {
    printk(KERN_DEBUG "iob=%llu \t nib=%llu \t x=%llu \t n=%.*s \n", 
        inode_offset_byte, h_sb->next_inode_byte, x,
        dentry->d_name.len, dentry->d_name.name);
    print_h_inode("", bucket_h_inode);
    break;
}
